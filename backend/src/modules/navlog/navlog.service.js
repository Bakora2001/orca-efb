 ...
 * navlog.service.js
 * ─────────────────
 * Builds a leg-by-leg navlog from an ordered waypoints array.
 * Each waypoint is either an airport { kind:'airport', id } or
 * a navpoint fix { kind:'fix', id }.
 */
...
export async function buildNavlog(aircraftId, waypoints) {
  ...
  // ── Resolve each waypoint to { ident, lat, lon } ─────────────
  const resolved = []
  ...
  const note = legs.length === 0
    ? 'No legs computed.'
    : `${legs.length} leg${legs.length > 1 ? 's' : ''} — ${Math.round(totalDistNm)} NM — ${minsToHHMM(totalEteMin)} ETE`
