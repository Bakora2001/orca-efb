...
def fit_quadratic_pure_python(xs, ys):
    n = len(xs)
    if n < 3:
        return None
    # Outlier filter: remove points further than 120px from a simple linear trend
    sum_x = sum(xs)
    sum_y = sum(ys)
    ...
    if abs(denom) > 1e-5:
        ...
        for x, y in zip(xs, ys):
            if abs(y - (slope * x + intercept)) < 120:
                clean_xs.append(x)
                clean_ys.append(y)
        if len(clean_xs) >= 3:
            xs, ys = clean_xs, clean_ys
            n = len(xs)
    ...
    sum_yx2 = sum(y*x*x for x, y in zip(xs, ys))
    ...
    if abs(d) < 1e-5:
        ...
        if abs(denom) < 1e-5:
            return None
        m = (n * sum_yx - sum_x * sum_y) / denom
        c = (sum_y - m * sum_x) / n
        return (0.0, m, c)

    da = (sum_yx2 * (sum_x2 * n - sum_x * sum_x) -
          sum_x3 * (sum_yx * n - sum_y * sum_x) +
          sum_x2 * (sum_yx * sum_x - sum_y * sum_x2))

    db = (sum_x4 * (sum_yx * n - sum_y * sum_x) -
          sum_yx2 * (sum_x3 * n - sum_x2 * sum_x) +
          sum_x2 * (sum_x3 * sum_y - sum_x2 * sum_yx))

    dc = (sum_x4 * (sum_x2 * sum_y - sum_x * sum_yx) -
          sum_x3 * (sum_x3 * sum_y - sum_x2 * sum_yx) +
          sum_yx2 * (sum_x3 * sum_x - sum_x2 * sum_x2))

    return (da / d, db / d, dc / d)

def evaluate_quadratic(coefs, x):
    a, b, c = coefs
    return a * x * x + b * x + c
...
def main():
    ...
    raw_rtow = data.get("rtow_kg")
    rtow_kg = float(raw_rtow) if raw_rtow is not None else None
    ...
    # Try loading a premium TrueType font (Arial) at readable sizes for the high-res image
    try:
        font = ImageFont.truetype("arial.ttf", 30)
        font_large = ImageFont.truetype("arial.ttf", 40)
        font_badge = ImageFont.truetype("arial.ttf", 34)
    except Exception:
    ...
    def fit_pressure_altitude_curves(cal_data):
        altitude_curves = cal_data["curve_families"]["pressure_altitude_curves"].get("altitude_curve_points") or []
        fitted = {}
        for curve in altitude_curves:
            try:
                ...
                if len(pts) >= 3:
                    xs = [float(p["x_px"]) for p in pts]
                    ys = [float(p["y_px"]) for p in pts]
                    coefs = fit_quadratic_pure_python(xs, ys)
                    if coefs:
                        fitted[alt] = coefs
    ...
    def generate_calibrated_pressure_altitude_curve(cal_data, start_x, end_x, altitude_ft, fitted_curves):
        path = []
        s_x = min(start_x, end_x)
        e_x = max(start_x, end_x)
        steps = int(abs(e_x - s_x)) + 1
        steps = max(2, min(steps, 400))
        alts = sorted(fitted_curves.keys())
        if not alts:
            return []
        for i in range(steps):
            ...
            x = s_x + t * (e_x - s_x)
            oat_c = oat_from_x(x)
            feed_alt = pressure_altitude_feed_ft(altitude_ft, oat_c)
            if feed_alt <= alts[0]:
                y = evaluate_quadratic(fitted_curves[alts[0]], x)
            elif feed_alt >= alts[-1]:
                y = evaluate_quadratic(fitted_curves[alts[-1]], x)
            else:
                for alt1, alt2 in zip(alts, alts[1:]):
                    if alt1 <= feed_alt <= alt2:
                        coefs1 = fitted_curves[alt1]
                        coefs2 = fitted_curves[alt2]
                        frac = (feed_alt - alt1) / (alt2 - alt1) if alt2 != alt1 else 0.0
                        y1 = evaluate_quadratic(coefs1, x)
                        y2 = evaluate_quadratic(coefs2, x)
                        y = y1 + frac * (y2 - y1)
                        break
            path.append((x, y))
        if start_x > end_x:
    ...
    def reviewed_weight_family_path(start_x, start_y, end_x, target_y, oat_c=None, samples=128):
        """
        Trace the weight-family curve from (start_x, start_y) to (end_x, target_y)
        so that it stays visually parallel to the printed background curves.

        Strategy: use the calibrated parallel_slope_dy_dx as the tangent direction
        at BOTH endpoints of a cubic Bezier. This means the drawn curve departs from
        the Reference Line at the same angle as the printed curves and arrives at the
        distance axis at the same angle — guaranteeing it never crosses neighbours.
        """
        ...
        # Read the calibrated slope (dy/dx in pixel space) for the weight family curves.
        # This is measured from the actual printed red lines and gives the correct angle.
        wf_cal = cal["curve_families"].get("weight_family_curves", {})
        slope = float(wf_cal.get("parallel_slope_dy_dx") or (dy / dx if abs(dx) > 0 else -0.87))

        # Build a cubic Bezier whose tangents at P0 and P3 match `slope`.
        # This makes the curve depart and arrive at the exact angle of the printed lines.
        # Control point distance along the tangent: 1/3 of the horizontal span.
        tang_len = dx / 3.0
        p0 = (start_x, start_y)
        p3 = (end_x,   target_y)
        # P1 is one third along the slope from P0
        p1 = (p0[0] + tang_len, p0[1] + slope * tang_len)
        # P2 is one third back along the slope from P3
        p2 = (p3[0] - tang_len, p3[1] - slope * tang_len)
        ...
        # Monotonicity clamp — prevent any tiny backwards wiggle from floating-point
        if dy <= 0:  # curve rises on screen (y decreasing)
            prev = start_y
            for i in range(len(path)):
                ...
                path[i] = (x, y)
                prev = y
        else:        # curve descends on screen (y increasing)
            prev = start_y
            for i in range(len(path)):
                x, y = path[i]
                y = max(y, prev)
    ...
    # Fit smooth quadratic curves to the weight-family envelope boundaries.
    # These are used inside reviewed_weight_family_path (closure) to trace
    # the drawn path along the true curvature of the printed nomograph lines.
    _wf_xs   = [float(p["x_px"]) for p in weight_family if p.get("y_min_px") is not None and p.get("y_max_px") is not None]
    _wf_ymin = [float(p["y_min_px"]) for p in weight_family if p.get("y_min_px") is not None and p.get("y_max_px") is not None]
    _wf_ymax = [float(p["y_max_px"]) for p in weight_family if p.get("y_min_px") is not None and p.get("y_max_px") is not None]
    coefs_w_min = fit_quadratic_pure_python(_wf_xs, _wf_ymin) if len(_wf_xs) >= 3 else None
    coefs_w_max = fit_quadratic_pure_python(_wf_xs, _wf_ymax) if len(_wf_xs) >= 3 else None
    ...
    pa_curve_pts = generate_calibrated_pressure_altitude_curve(
        cal, start_curve_x, temp_x, elev_ft, fitted_curves
    )

    locked_y = _locked_shared_pressure_altitude_y(cal, oat, elev_ft, icao)
    if locked_y is not None:
        pressure_y = locked_y
        if pa_curve_pts:
            offset_y = pressure_y - float(pa_curve_pts[-1][1])
            pa_curve_pts = [(x, y + offset_y) for x, y in pa_curve_pts]
    else:
        if pa_curve_pts:
            pressure_y = float(pa_curve_pts[-1][1])
        else:
